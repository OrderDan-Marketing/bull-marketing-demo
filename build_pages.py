...
# ================= CRM (可芳) — 內容來源:projects\BNI行銷產業鏈\可芳\BNI簡報.pdf(15 頁,逐字稿 pdf_transcript.md)+ 1.jpg 人像(2026-09-15) =================
crm_after_hero = f'''
<section>
  <div class="wrap">
    <div class="eyebrow">Member Journey</div>
    <h2 class="h2">吸客、育客、回購、轉介紹</h2>
    <p class="lead">一站式會員經營,讓生意自然成長——四個步驟,把陌生客變成會員、變成回購、變成幫你介紹的人。</p>
    {note_grid([
        ('01', '集客建會員', '便利一鍵集客,完整用戶資料。', ['一鍵集客', '用戶資料']),
        ('02', '育客增信任', '依照訪客行為,走不同的育客路徑。', ['行為分眾', '育客路徑']),
        ('03', '鎖客增銷量', '點數、大禮包,單次與多次滿額。', ['點數', '滿額禮']),
        ('04', '拓客增客戶', '一次轉介紹,讓顧客成為你的業務。', ['轉介紹']),
    ])}
    <div class="vd-hl" style="grid-template-columns:1fr;margin-top:22px"><div class="reveal"><small>LINE OFFICIAL ACCOUNT</small><b>「我們是一套整合 LINE 官方帳號的會員系統。」</b>{chips(['精準行銷', '提升營收', '會員經營'])}</div></div>
  </div>
</section>

<section style="background:#fff">
  <div class="wrap">
    <div class="eyebrow">3 Stages</div>
    <h2 class="h2">會員裂變行銷三階段:全自動的獲客引擎</h2>
    <p class="lead">從「買廣告」到「造節點」。每一階段都有自動機制,不用人工盯。</p>
    {vd_cards([
        ('階段一|銷售前', '分享裂變/獲客、引客流', ['<b>自動貼標籤</b>:加好友當下就分眾', '設定誘因:如加好友送肉盤券、送現金抵用券', '廣告與短影音導進來的人,第一步就留下']),
        ('階段二|銷售中', '募集會員/提高日營業額', ['<b>自動發獎勵</b>:消費就累點、就升級', '任務設定:滿額升級 VIP、解鎖抽獎', '店內消費客戶自動發問券,填寫即抽獎']),
        ('階段三|銷售後', '分銷互利/任務達成', ['<b>推薦機制</b>:每邀 3 人再送積分', '按讚裂變、揪團集好友送折價券', '讓顧客成為你的業務']),
    ])}
  </div>
</section>
...
<section>
  <div class="wrap">
    ...
    <h2 class="h2">工具多元的時代,整合能力決定行銷效率</h2>
    <p class="lead">賦能型整合行銷工具,讓企業會員資料整合更完善,打破資料孤島。</p>
    <div class="vd-formula">
      <div class="quote reveal"><p>沒有經營 = 客戶來了就走了<br>有了官方 LINE = 客戶不斷回來</p><small>不要讓你的客戶變成一次性</small></div>
      <div class="vd-card reveal"><h3>可以接進來的系統</h3><p>會員資料不再散在六個後台;POS、訂位、購物車、派工、開課,全部回到同一個 LINE 會員池。</p>{chips(['餐飲 POS+線上訂位系統', '美業 POS+預約系統', '連鎖總部系統', '電商購物車', '工程派工系統', '影音開課平台'])}</div>
    </div>
    <div class="vd-grid3" style="margin-top:22px">
      <div class="vd-card reveal"><h3>會員管理 × 微網頁</h3><p>表單問卷、微網頁文章、互動試算——<a href="web.html">網站表單</a>直接進 LINE 自動貼標。</p></div>
      <div class="vd-card reveal"><h3>優惠券 × 點數 × 抽獎</h3><p>現金抵用券、兌換券、生日禮、VIP 升級、抽獎,全部自動發、自動核銷。</p></div>
      <div class="vd-card reveal"><h3>數據回到同一張表</h3><p>會員數、互動數、核銷率、預約量都在後台;<a href="ads.html">廣告端</a>拿去做再行銷。</p></div>
    </div>
  </div>
</section>
'''
...
crm_hl = [
    ('精品咖啡', '每月營業額破 150 萬', ['上線次月起連續 13 個月', '月訂單總額 151 萬~209 萬']),
    ('台式牛排餐飲', '會員數 不到兩年破 4 萬人', ['會員數 40,809', '優惠券核銷率 98%']),
    ('珠寶零售', '啟動不到一個月 會員 +1,000 位', ['好友人數 6,814', '付費會員 475 位|5 個月 474,525']),
]
...
{vd_cards([
    ('精品咖啡', '會員快速增長 → 提高日營業額 → 讓客戶變成你的業務', ['<b>銷售前</b>:點餐完成即獲得 50 元現金抵用券 10 張、雞翅兌換券 8 張;每推薦一位好友得紅利 1 點', '<b>銷售中</b>:每消費 30 元得 1 點(可兌換私房料理);購買 99 VIP 專案得 99 杯特調氣泡飲(90 天有效)', '<b>銷售後</b>:店消費客戶自動發問券,填寫可抽獎;集滿 10 人按讚即贈雞翅兌換券']),
    ('珠寶零售', '7/14 正式啟動,第一階段不到一個月 +1,000 位會員', ['<b>銷售前</b>:加入官方 LINE 贈 $3,000 禮券(2,000/600/200/100 各 1 張、50 元 2 張)', '<b>銷售中</b>:消費滿 3,000 送 3,000、滿 9,000 送 9,000 依此類推;滿 $5,000 抽獎一次', '<b>銷售後</b>:消費客戶自動發問券,填寫贈 300 現金抵用券;揪團集 10 位好友送 50 元折價券', '<b>付費會員</b>:入會六大好禮(首次入會贈鑽石項鍊、推薦好友領 $300、消費再享 10% 回饋…)']),
    ('采耳・頭部 SPA 美業', '預約系統+會員專區,客戶黏著度極高', ['<b>預約列表</b>:1,242 筆預約,採耳、耳燭套餐、頭部舒壓 SPA 隨機或指定技師', '<b>會員專區</b>:VIP、會員兌換、我的票券、分享拿好禮、抽好禮', '<b>服務資訊</b>:新品服務體驗價優惠券,自動回覆推播']),
])}
<h3 style="font-size:24px;margin-top:56px">更多案例</h3>
{ind_grid([
    ('餐飲業', '台式牛排連鎖門市', ['鄰居活動滿額送券', '兌換優惠券後台', '核銷率 98%']),
    ('美業', '采耳培訓所', ['預約 1,242 筆', '三層選單']),
    ('零售業', '翡翠珠寶品牌', ['滿百打 95 折', '現金回饋會員 475 位']),
    ('個人業務', '財富設計師(保險理財)', ['自動回覆選單', '微網頁理財文章', '退休金互動試算']),
])}'''
...
<section>
  <div class="wrap">
    ...
    <h2 class="h2">百行百業都需要官方 LINE 收集客戶</h2>
    <p class="lead">把陌生客戶變成你的長期顧客,經營才會有價值。各行各業都適用。</p>
    {note_grid([
        ('零售門市', '提升回購率', '新品通知、會員優惠。', None),
        ('餐飲美食', '培養忠實顧客', '訂位提醒、菜單推薦。', None),
        ('美容美業', '提高續約率', '預約提醒、療程關懷。', None),
        ('教育培訓', '精準招生不流失', '課程諮詢、開課通知。', None),
        ('健身運動', '提升出席率', '課程提醒、活動邀約。', None),
        ('醫療健康', '提升服務品質', '掛號提醒、衛教推播。', None),
        ('汽車服務', '增加回廠率', '保養提醒、活動推播。', None),
        ('房地產', '提高成交機會', '物件推薦、預約看屋。', None),
        ('婚禮攝影', '提升預約率', '檔期諮詢、作品分享。', None),
        ('更多產業', '都能有效經營客戶', '各式服務業、B2B。', None),
    ])}
    <h3 style="font-size:24px;margin-top:56px">已有成功案例的產業</h3>
    <p class="lead">針對不同產業痛點,量身打造【銷售前、中、後】的裂變戰術手冊。</p>
    <div class="chips vd-ips">{''.join(f'<span>{H.escape(x)}</span>' for x in ['餐飲業', '零售/藥局', '美業/醫美', '娛樂/KTV', '汽車美容', '旅遊業'])}</div>
    <h3 style="font-size:24px;margin-top:56px">特別適合</h3>
    <div class="vd-hl">
      <div class="reveal"><small>餐飲業</small><b>火鍋店、燒肉店、牛排館、日式料理</b>{chips(['訂位', '滿額送券', '集點'])}</div>
      <div class="reveal"><small>美業</small><b>醫美診所、美業門店</b>{chips(['預約', '療程關懷', 'VIP'])}</div>
      <div class="reveal"><small>零售業</small><b>藥局、公仔店、珠寶</b>{chips(['會員優惠', '現金回饋', '新品通知'])}</div>
    </div>
  </div>
</section>
...
<section style="background:#fff">
  <div class="wrap">
    <div class="eyebrow">Take Action</div>
    <h2 class="h2">想了解你的行業適不適合做?</h2>
    <p class="lead">歡迎一對一諮詢,我幫你健檢。</p>
    {steps_html([
        ('分析你的產業痛點', '你的客人現在是來一次就走,還是有在回來?先看你的客流與回購長什麼樣。', ['產業痛點']),
        ('提供專屬建議方案', '依門市數、系統現況與預算,給你銷售前、中、後的設定與方案級別。', ['專屬方案']),
        ('讓你少走彎路、快速成長', '上線、誘因設定、自動化都由我們接手;你只要看後台數字。', ['快速上線']),
    ])}
    <p class="lead" style="margin-top:26px">不要讓你的客戶變成一次性。 <a class="btn btn-gold" href="#contact" style="margin-left:10px">預約免費診斷 →</a></p>
  </div>
</section>
'''

page('crm', 'LINE 會員系統', '可芳', 'CRM',
     '從「買廣告」到「造節點」,裂變系統把獲客成本降低 90%。廣告導進來的每個人都留在你的私域池——會員管理、表單問卷、微網頁、優惠券點數自動行銷,一套整合 LINE 官方帳號的無縫流動會員生態系。',
     '張瑜芳。打造會員經營與裂變成長的實戰操盤者;網路銷售批發起家、保養品批發百人團隊領導經驗;鼎立國際貿易有限公司執行長、屹登股份有限公司副總經理、2025 年新高雄創新創業協會理事長;《新時代年輕履歷》2024 年度暢銷書作家。',
     [('40,000+', '單一餐飲客戶會員數(不到兩年)'), ('98%', '優惠券核銷率'), ('1,242 筆', '單一美業客戶預約量')],
     [
